<?php
// church/modules/church_records/wedding/series_records.php
// Phase 4 — Step 4.10: Records list inside a wedding year series
...
$s = $conn->prepare("SELECT id, notes FROM wedding_series WHERE series_year = ?");
$s->bind_param("i", $year);
$s->execute();
...
if (!$series) {
    $_SESSION['error'] = "No wedding series found for {$year}.";
    header('Location: /church/modules/church_records/wedding/series_list.php');
    exit;
}

$page_title = "Wedding Records — {$year}";
...
// ── Build WHERE clause ───────────────────────────────────────
$where_parts = ["wr.is_archived = 0", "wr.series_year = ?"];
$params      = [$year];
$types       = "i";

if ($search !== '') {
    $where_parts[] = "(wr.groom_name LIKE ? OR wr.bride_name LIKE ? OR wr.record_no LIKE ? OR wr.minister LIKE ?)";
    $like = "%{$search}%";
...
if ($filter_doc === 'yes') {
    $where_parts[] = "(wr.document_path IS NOT NULL AND wr.document_path != '')";
} elseif ($filter_doc === 'no') {
    $where_parts[] = "(wr.document_path IS NULL OR wr.document_path = '')";
}
...
$order_sql = match($sort) {
    'groom_asc'  => 'wr.groom_name ASC',
    'groom_desc' => 'wr.groom_name DESC',
    'bride_asc'  => 'wr.bride_name ASC',
    'date_asc'   => 'wr.date_of_wedding ASC',
    default      => 'wr.date_of_wedding DESC',
};

// ── Total count for pagination ───────────────────────────────
$count_stmt = $conn->prepare("SELECT COUNT(*) FROM wedding_records wr WHERE {$where_sql}");
...
$fetch_sql = "
    SELECT
        ...
        wr.bride_name,
        wr.date_of_wedding,
        wr.principal_sponsor_male,
        wr.principal_sponsor_female,
        wr.witness1_name,
        wr.witness2_name,
        wr.minister,
        wr.document_path,
        wr.created_at
    FROM wedding_records wr
    WHERE {$where_sql}
    ORDER BY {$order_sql}
    LIMIT ? OFFSET ?
";
...
$stats_r = $conn->prepare("
    SELECT
        ...
        MIN(date_of_wedding)                                             AS first_date,
        MAX(date_of_wedding)                                             AS last_date
...
<style>
    ...
    .shc-title {
        font-family: 'Playfair Display', serif;
        font-size: 1.15rem; color: #fff; font-weight: 600; margin-bottom: 3px;
    }
    ...
    .shc-stat-value {
        font-family: 'Playfair Display', serif;
        font-size: 1.55rem; color: #fcd34d; font-weight: 600; line-height: 1;
    }
    .shc-stat-label {
    ...
    .scan-strip-bar {
        ...
        height: 7px; background: #f3ede3; border-radius: 99px; overflow: hidden;
    }
    .scan-strip-fill {
        height: 100%;
    ...
    .scan-strip-pct {
        font-size: 0.8rem; font-weight: 600; color: #d97706; white-space: nowrap;
    }
    ...
    .search-wrap i {
        ...
        transform: translateY(-50%);
        color: #c4b89a; font-size: 0.82rem; pointer-events: none;
    }
    .search-input {
        width: 100%;
    ...
    .records-table-title i { color: #d97706; font-size: 0.8rem; }
    .records-count-badge {
    ...
    .couple-cell { display: flex; align-items: flex-start; gap: 8px; }
    .couple-icon {
    ...
    .action-view  { color: #3b82f6; background: #eff6ff; }
    ...
    .empty-icon {
        ...
        background: #fef3c7;
        display: flex; align-items: center; justify-content: center;
        margin: 0 auto 20px; color: #d97706; font-size: 2rem;
    }

    /* ── Pagination ── */
    .pagination {
        display: flex; align-items: center; justify-content: space-between;
        padding: 14px 20px; border-top: 1px solid #f0ebe0;
        background: #fdfcfa;
    ...
    .page-btn {
        ...
        border: 1px solid #d1cdc4; color: #6b7280; background: #fff;
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    <div class="page-header-left">
        <h1>
            <i class="fas fa-ring" style="color:#d97706;margin-right:8px;font-size:1rem;"></i>
            Wedding — <?= $year ?>
        </h1>
        <p class="breadcrumb">
    ...
    <a href="/church/modules/church_records/wedding/add_record.php?year=<?= $year ?>"
       class="toolbar-btn btn-amber">
...
<div style="padding:24px 28px 60px; width:100%; box-sizing:border-box;">
    ...
    <div class="series-header-card">
        <div style="display:flex;align-items:center;gap:18px;">
            <div class="shc-icon"><i class="fas fa-ring"></i></div>
            <div>
                <div class="shc-title"><?= $year ?> Wedding Series</div>
                <div class="shc-sub">
        ...
        <div class="shc-stats">
            <div class="shc-stat">
                <div class="shc-stat-value"><?= number_format($stats['total']) ?></div>
                <div class="shc-stat-label">Records</div>
            </div>
            <div class="shc-divider"></div>
            <div class="shc-stat">
                <div class="shc-stat-value"><?= $stats['scanned'] ?></div>
                <div class="shc-stat-label">Scanned</div>
            </div>
            <div class="shc-divider"></div>
            <div class="shc-stat">
                <div class="shc-stat-value"><?= $scan_pct ?>%</div>
                <div class="shc-stat-label">Coverage</div>
            </div>
        </div>
    </div>
    ...
    <div class="scan-strip">
        <div class="scan-strip-label">
            <i class="fas fa-file-image" style="color:#d97706;"></i>
            Document scan coverage
        </div>
        <div class="scan-strip-bar">
            <div class="scan-strip-fill" style="width:<?= $scan_pct ?>%;"></div>
        </div>
        <div class="scan-strip-pct"><?= $stats['scanned'] ?> / <?= $stats['total'] ?> scanned</div>
        <?php if ($scan_pct < 100): ?>
        <div style="font-size:0.72rem;color:#9ca3af;">
            <?= $stats['total'] - $stats['scanned'] ?> record<?= ($stats['total'] - $stats['scanned']) !== 1 ? 's' : '' ?> missing a scan
        </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
        ...
        <div class="toolbar">
            ...
            <div class="search-wrap">
                ...
                <input type="text"
                       name="q"
                       class="search-input"
                       placeholder="Search groom, bride, minister…"
            ...
            <select name="sort" class="filter-select" onchange="this.form.submit()">
                ...
                <option value="groom_asc" <?= $sort === 'groom_asc' ? 'selected' : '' ?>>Groom A–Z</option>
                <option value="bride_asc" <?= $sort === 'bride_asc' ? 'selected' : '' ?>>Bride A–Z</option>
            </select>

            <button type="submit" class="toolbar-btn btn-amber">
    ...
    <div class="records-table-wrap">
        ...
        <div class="records-table-header">
            <div class="records-table-title">
                ...
                <?php if ($search !== '' || $filter_doc !== ''): ?>
                <span style="font-size:0.72rem;color:#9ca3af;font-family:'DM Sans',sans-serif;font-weight:400;">
                    — filtered results
                </span>
                <?php endif; ?>
            </div>
        ...
        <div class="empty-state">
            <div class="empty-icon"><i class="fas fa-ring"></i></div>
            <?php if ($search !== '' || $filter_doc !== ''): ?>
                <h3 style="font-family:'Playfair Display',serif;font-size:1rem;color:#0f2044;margin-bottom:8px;">No matching records</h3>
                <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;">No wedding records match your search. Try adjusting your filters.</p>
                <a href="?year=<?= $year ?>" class="toolbar-btn btn-outline" style="display:inline-flex;">
            ...
            <?php else: ?>
                ...
                <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;max-width:340px;margin-left:auto;margin-right:auto;line-height:1.6;">
                    This series is empty. Add the first wedding record for <?= $year ?>.
                </p>
                <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                <a href="/church/modules/church_records/wedding/add_record.php?year=<?= $year ?>"
                   class="toolbar-btn btn-amber" style="display:inline-flex;">
                ...
                <tr>
                    ...
                    <td>
                        <div class="couple-cell">
                            <div class="couple-icon" style="background:#eff6ff;color:#3b82f6;">
                                <i class="fas fa-mars"></i>
                            </div>
                            <div>
                                <div class="record-name"><?= htmlspecialchars($rec['groom_name']) ?></div>
                                <?php if ($rec['minister']): ?>
                                <div class="record-sub">
                                    <i class="fas fa-cross" style="font-size:0.55rem;margin-right:3px;"></i>
                                    <?= htmlspecialchars($rec['minister']) ?>
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <td>
                        <div class="couple-cell">
                            <div class="couple-icon" style="background:#fdf2f8;color:#ec4899;">
                                <i class="fas fa-venus"></i>
                            </div>
                            <div>
                                <div class="record-name"><?= htmlspecialchars($rec['bride_name']) ?></div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td style="font-size:0.8rem;color:#374151;white-space:nowrap;">
                        <?= $rec['date_of_wedding']
                            ? date('M j, Y', strtotime($rec['date_of_wedding']))
                    ...
                    <td>
                        <?php
                            $sp_m = $rec['principal_sponsor_male']   ?? null;
                            $sp_f = $rec['principal_sponsor_female'] ?? null;
                            $w1   = $rec['witness1_name']            ?? null;
                            $w2   = $rec['witness2_name']            ?? null;
                        ?>
                        <?php if ($sp_m || $sp_f): ?>
                        <div style="font-size:0.78rem;color:#374151;">
                            <?php if ($sp_m): ?>
                            <div style="display:flex;align-items:center;gap:4px;">
                                <i class="fas fa-hands-praying" style="color:#b8933a;font-size:0.6rem;flex-shrink:0;"></i>
                                <?= htmlspecialchars($sp_m) ?>
                            </div>
                            <?php endif; ?>
                            <?php if ($sp_f): ?>
                            <div style="color:#9ca3af;font-size:0.73rem;margin-top:2px;">
                                <?= htmlspecialchars($sp_f) ?>
                            </div>
                            <?php endif; ?>
                            <?php
                                $witnesses = array_filter([$w1, $w2]);
                                if ($witnesses):
                            ?>
                            <div style="color:#c4b89a;font-size:0.7rem;margin-top:3px;">
                                <?= htmlspecialchars(implode(', ', $witnesses)) ?>
                            </div>
                            <?php endif; ?>
                        </div>
                        <?php else: ?>
                    ...
                    <td style="text-align:center;white-space:nowrap;">
                        <div style="display:flex;align-items:center;justify-content:center;gap:5px;">
                            <a href="/church/modules/church_records/wedding/view_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-view" title="View">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                            <a href="/church/modules/church_records/wedding/edit_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-edit" title="Edit">
                                <i class="fas fa-pen"></i>
                            </a>
                            <a href="/church/modules/church_records/wedding/print_certificate.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-print" title="Print Certificate"
                               target="_blank">
        ...
        <div class="pagination">
            ...
            <div class="pagination-links">
                ...
                <?php
                $window_start = max(1, $page_num - 2);
                $window_end   = min($total_pages, $page_num + 2);
                if ($window_start > 1): ?>
                    <a href="<?= page_url(1, $year, $search, $filter_doc, $sort) ?>" class="page-btn">1</a>
                    <?php if ($window_start > 2): ?>
                    <span class="page-btn" style="pointer-events:none;border:none;color:#c4b89a;">…</span>
                    <?php endif;
                endif; ?>
                ...
                <?php if ($window_end < $total_pages): ?>
                    ...
                    <span class="page-btn" style="pointer-events:none;border:none;color:#c4b89a;">…</span>
                    <?php endif; ?>
                    <a href="<?= page_url($total_pages, $year, $search, $filter_doc, $sort) ?>" class="page-btn">
                        <?= $total_pages ?>
                    </a>
                <?php endif; ?>
