<?php
// church/modules/church_records/funeral/edit_series.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $check = $conn->prepare("SELECT COUNT(*) FROM funeral_records WHERE series_year = ? AND is_archived = 0");
        ...
        if ($cnt > 0) {
            $_SESSION['error'] = "Cannot delete series {$series_year} because it still has funeral records.";
            header("Location: /church/modules/church_records/funeral/edit_series.php?year={$series_year}");
            exit;
        }

        $del = $conn->prepare("DELETE FROM funeral_series WHERE series_year = ?");
        $del->bind_param("i", $series_year);
        $del->execute();
        $del->close();

        $rs = $conn->prepare("DELETE FROM record_series WHERE type = 'funeral' AND year = ?");
        $rs->bind_param("i", $series_year);
        $rs->execute();
        $rs->close();

        $_SESSION['success'] = "Funeral series {$series_year} was removed.";
        header('Location: /church/modules/church_records/funeral/series_list.php');
        exit;
    }

    $upd = $conn->prepare("UPDATE funeral_series SET notes = ? WHERE series_year = ?");
    $upd->bind_param("si", $notes, $series_year);
    $upd->execute();
    $upd->close();

    $_SESSION['success'] = "Funeral series {$series_year} was updated.";
    header('Location: /church/modules/church_records/funeral/series_list.php');
    exit;
}
...
$stmt = $conn->prepare("SELECT id, series_year, notes FROM funeral_series WHERE series_year = ?");
...
$page_title = "Edit Funeral Series {$series['series_year']}";
...
<div style="max-width:640px;margin:32px auto 60px;padding:24px;border-radius:16px;background:#fff;border:1px solid #ede8de;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <h1 style="font-size:1.1rem;display:flex;align-items:center;gap:8px;margin:0;">
            <i class="fas fa-cross" style="color:#4b5563;"></i>
            Edit Funeral Series (<?= htmlspecialchars($series['series_year']) ?>)
        </h1>
    ...
    <form method="POST" action="/church/modules/church_records/funeral/edit_series.php" style="margin-bottom:20px;">
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:10px;">
            <button type="submit" name="action" value="update"
                    style="padding:8px 18px;border-radius:8px;border:none;background:#4b5563;
    ...
    <form method="POST" action="/church/modules/church_records/funeral/edit_series.php"
          onsubmit="return confirm('Are you sure you want to remove this series? It must have no funeral records.');">
        <input type="hidden" name="series_year" value="<?= htmlspecialchars($series['series_year']) ?>">
