<?php
// church/modules/church_records/communion/add_series.php
// Phase 4 — Communion Module: Handle new year series form submission
...
$check = $conn->prepare("SELECT id FROM communion_series WHERE series_year = ?");
...
// ── Insert new series ─────────────────────────────────────────
$stmt = $conn->prepare("
    INSERT INTO communion_series (series_year, notes, created_by, created_at)
    VALUES (?, ?, ?, NOW())
");
$stmt->bind_param("isi", $series_year, $notes, $_SESSION['user_id']);

if ($stmt->execute()) {
    $_SESSION['success'] = "Communion series for {$series_year} created successfully.";
    header("Location: /church/modules/church_records/communion/series_records.php?year={$series_year}");
} else {
