<?php
// church/modules/church_records/communion/edit_series.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $check = $conn->prepare("SELECT COUNT(*) FROM communion_records WHERE series_year = ? AND is_archived = 0");
        ...
        if ($cnt > 0) {
            $_SESSION['error'] = "Cannot delete series {$series_year} because it still has communion records.";
            header("Location: /church/modules/church_records/communion/edit_series.php?year={$series_year}");
            exit;
        }

        $del = $conn->prepare("DELETE FROM communion_series WHERE series_year = ?");
        $del->bind_param("i", $series_year);
        $del->execute();
        $del->close();

        $_SESSION['success'] = "Communion series {$series_year} was removed.";
        header('Location: /church/modules/church_records/communion/series_list.php');
        exit;
    }

    $upd = $conn->prepare("UPDATE communion_series SET notes = ? WHERE series_year = ?");
    $upd->bind_param("si", $notes, $series_year);
    $upd->execute();
    $upd->close();

    $_SESSION['success'] = "Communion series {$series_year} was updated.";
    header('Location: /church/modules/church_records/communion/series_list.php');
    exit;
}
...
$stmt = $conn->prepare("SELECT id, series_year, notes FROM communion_series WHERE series_year = ?");
...
$page_title = "Edit Communion Series {$series['series_year']}";
...
<div style="max-width:640px;margin:32px auto 60px;padding:24px;border-radius:16px;background:#fff;border:1px solid #ede8de;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <h1 style="font-size:1.1rem;display:flex;align-items:center;gap:8px;margin:0;">
            <i class="fas fa-bread-slice" style="color:#e11d74;"></i>
            Edit Communion Series (<?= htmlspecialchars($series['series_year']) ?>)
        </h1>
    ...
    <form method="POST" action="/church/modules/church_records/communion/edit_series.php" style="margin-bottom:20px;">
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:10px;">
            <button type="submit" name="action" value="update"
                    style="padding:8px 18px;border-radius:8px;border:none;background:#059669;
    ...
    <form method="POST" action="/church/modules/church_records/communion/edit_series.php"
          onsubmit="return confirm('Are you sure you want to remove this series? It must have no communion records.');">
        <input type="hidden" name="series_year" value="<?= htmlspecialchars($series['series_year']) ?>">
