<?php
// church/modules/church_records/funeral/series_records.php
...
$s = $conn->prepare("SELECT id, notes FROM funeral_series WHERE series_year = ?");
$s->bind_param("i", $year);
$s->execute();
$series = $s->get_result()->fetch_assoc();
$s->close();
if (!$series) {
    $_SESSION['error'] = "No funeral series found for {$year}.";
    header('Location: /church/modules/church_records/funeral/series_list.php');
    exit;
}

$page_title = "Funeral Records — {$year}";
...
// ── WHERE ────────────────────────────────────────────────────
$where_parts = ["fr.is_archived = 0", "fr.series_year = ?"];
$params = [$year];
$types  = "i";

if ($search !== '') {
    $where_parts[] = "(fr.deceased_name LIKE ? OR fr.record_no LIKE ? OR fr.next_of_kin LIKE ? OR fr.next_of_kin_name LIKE ?)";
    $like = "%{$search}%";
    $params = array_merge($params, [$like, $like, $like, $like]);
    $types .= "ssss";
}
if ($filter_doc === 'yes') {
    $where_parts[] = "(fr.document_path IS NOT NULL AND fr.document_path != '')";
} elseif ($filter_doc === 'no') {
    $where_parts[] = "(fr.document_path IS NULL OR fr.document_path = '')";
}
...
$order_sql = match($sort) {
    'name_asc'   => 'fr.deceased_name ASC',
    'name_desc'  => 'fr.deceased_name DESC',
    'death_asc'  => 'fr.date_of_death ASC',
    'date_asc'   => 'fr.date_of_funeral ASC',
    default      => 'fr.date_of_funeral DESC',
};

// ── Count ────────────────────────────────────────────────────
$cnt = $conn->prepare("SELECT COUNT(*) FROM funeral_records fr WHERE {$where_sql}");
$cnt->bind_param($types, ...$params);
$cnt->execute();
$total_records = (int)$cnt->get_result()->fetch_row()[0];
$cnt->close();
...
// ── Fetch ────────────────────────────────────────────────────
$stmt = $conn->prepare("
    SELECT fr.id, fr.record_no, fr.deceased_name,
           fr.date_of_birth, fr.date_of_death, fr.date_of_funeral,
           fr.next_of_kin, fr.next_of_kin_name, fr.next_of_kin_contact,
           fr.place_of_burial, fr.minister, fr.document_path
    FROM funeral_records fr
    WHERE {$where_sql}
    ORDER BY {$order_sql}
    LIMIT ? OFFSET ?
");
...
$stats_r = $conn->prepare("
    SELECT
        COUNT(*)                                                          AS total,
        COUNT(CASE WHEN document_path IS NOT NULL
                    AND document_path != '' THEN 1 END)                  AS scanned,
        MIN(date_of_funeral)                                             AS first_date,
        MAX(date_of_funeral)                                             AS last_date
...
function page_url($p, $year, $search, $filter_doc, $sort) {
    ...
    if ($filter_doc !== '')    $args['doc']  = $filter_doc;
...
<style>
    ...
    .shc-stat-value { font-family:'Playfair Display',serif;font-size:1.55rem;color:#d1d5db;font-weight:600;line-height:1; }
    ...
    .scan-strip-label { font-size:0.78rem;color:#6b7280;white-space:nowrap;display:flex;align-items:center;gap:6px; }
    .scan-strip-bar { flex:1;min-width:120px;height:7px;background:#f3ede3;border-radius:99px;overflow:hidden; }
    .scan-strip-fill { height:100%;background:linear-gradient(to right,#4b5563,#9ca3af);border-radius:99px;transition:width 0.5s ease; }
    .scan-strip-pct { font-size:0.8rem;font-weight:600;color:#4b5563;white-space:nowrap; }
    ...
    .search-input:focus { border-color:#4b5563;box-shadow:0 0 0 3px rgba(75,85,99,0.12); }
    .search-input::placeholder { color:#c4b89a; }
    .filter-select { padding:8px 10px;border:1px solid #d1cdc4;border-radius:8px;font-size:0.82rem;color:#374151;background:#fff;outline:none;cursor:pointer;transition:border-color 0.18s;font-family:'DM Sans',sans-serif; }
    .filter-select:focus { border-color:#4b5563; }
    ...
    .btn-slate { background:#4b5563;color:#fff; } .btn-slate:hover { background:#374151; }
    .btn-outline { background:#fff;color:#6b7280;border:1px solid #d1cdc4; } .btn-outline:hover { background:#faf7f0;border-color:#c4b89a;color:#0f2044; }
    .records-table-wrap { background:#fff;border:1px solid #d1cdc4;border-radius:14px;overflow:hidden;width:100%;box-shadow:0 2px 16px rgba(15,32,68,0.08); }
    ...
    .records-table-title i { color:#6b7280;font-size:0.8rem; }
    .records-count-badge { display:inline-flex;align-items:center;font-size:0.72rem;font-weight:600;background:#f3f4f6;color:#4b5563;padding:2px 9px;border-radius:99px; }
    ...
    .action-view  { color:#3b82f6;background:#eff6ff; } .action-view:hover  { background:#3b82f6;color:#fff; }
    .action-edit  { color:#b8933a;background:#fdf8ec; } .action-edit:hover  { background:#b8933a;color:#fff; }
    .action-print { color:#6b7280;background:#f3f4f6; } .action-print:hover { background:#6b7280;color:#fff; }
    .empty-state { text-align:center;padding:60px 24px; }
    .empty-icon { width:72px;height:72px;border-radius:20px;background:#f3f4f6;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;color:#6b7280;font-size:2rem; }
    ...
    .page-btn:hover { border-color:#4b5563;color:#4b5563; }
    .page-btn.active { background:#4b5563;color:#fff;border-color:#4b5563; }
    ...
    @media(max-width:768px){ .shc-stats{display:none;} .toolbar{flex-direction:column;align-items:stretch;} .search-wrap{max-width:100%;} .action-print{display:none;} }
</style>

<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    <div class="page-header-left">
        <h1><i class="fas fa-cross" style="color:#6b7280;margin-right:8px;font-size:1rem;"></i>Funeral — <?= $year ?></h1>
        <p class="breadcrumb">
    ...
    <a href="/church/modules/church_records/funeral/add_record.php?year=<?= $year ?>"
       class="toolbar-btn btn-slate">
...
<div style="padding:24px 28px 60px; width:100%; box-sizing:border-box;">
    ...
    <div class="series-header-card">
        <div style="display:flex;align-items:center;gap:18px;">
            <div class="shc-icon"><i class="fas fa-cross"></i></div>
            <div>
                <div class="shc-title"><?= $year ?> Funeral Series</div>
                <div class="shc-sub">
                    <?php if ($stats['first_date'] && $stats['last_date']): ?>
                        <?= date('M j', strtotime($stats['first_date'])) ?> – <?= date('M j, Y', strtotime($stats['last_date'])) ?>
                    <?php else: ?>No records yet<?php endif; ?>
                    <?php if (!empty($series['notes'])): ?>&nbsp;·&nbsp; <?= htmlspecialchars($series['notes']) ?><?php endif; ?>
                </div>
            </div>
        </div>
        <div class="shc-stats">
    ...
    <div class="scan-strip">
        <div class="scan-strip-label"><i class="fas fa-file-image" style="color:#6b7280;"></i>Document scan coverage</div>
        ...
        <div class="toolbar">
            <div class="search-wrap">
                ...
                <input type="text" name="q" class="search-input"
                       placeholder="Search deceased, next of kin…"
            ...
            <select name="sort" class="filter-select" onchange="this.form.submit()">
                <option value="date_desc" <?= $sort==='date_desc' ?'selected':'' ?>>Funeral Date ↓</option>
                <option value="date_asc"  <?= $sort==='date_asc'  ?'selected':'' ?>>Funeral Date ↑</option>
                <option value="name_asc"  <?= $sort==='name_asc'  ?'selected':'' ?>>Name A–Z</option>
                <option value="name_desc" <?= $sort==='name_desc' ?'selected':'' ?>>Name Z–A</option>
            </select>
            <button type="submit" class="toolbar-btn btn-slate"><i class="fas fa-magnifying-glass"></i> Search</button>
            <?php if ($search !== '' || $filter_doc !== ''): ?>
            <a href="?year=<?= $year ?>" class="toolbar-btn btn-outline"><i class="fas fa-times"></i> Clear</a>
            <?php endif; ?>
            <div style="flex:1;"></div>
            <a href="/church/modules/church_records/funeral/series_list.php" class="toolbar-btn btn-outline" title="Back to series list">
    ...
    <div class="records-table-wrap">
        ...
        <div class="empty-state">
            <div class="empty-icon"><i class="fas fa-cross"></i></div>
            <?php if ($search !== '' || $filter_doc !== ''): ?>
                <h3 style="font-family:'Playfair Display',serif;font-size:1rem;color:#0f2044;margin-bottom:8px;">No matching records</h3>
                <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;">No funeral records match your search. Try adjusting your filters.</p>
            ...
            <?php else: ?>
                ...
                <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;max-width:340px;margin-left:auto;margin-right:auto;line-height:1.6;">This series is empty. Add the first funeral record for <?= $year ?>.</p>
                <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                <a href="/church/modules/church_records/funeral/add_record.php?year=<?= $year ?>" class="toolbar-btn btn-slate" style="display:inline-flex;">
        ...
        <div style="width:100%;overflow-x:auto;">
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        <div class="record-name"><?= htmlspecialchars($rec['deceased_name']) ?></div>
                        <?php if ($rec['date_of_birth']): ?>
                        <div class="record-sub">b. <?= date('M j, Y', strtotime($rec['date_of_birth'])) ?></div>
                        <?php endif; ?>
                        <?php if ($rec['minister']): ?>
                        <div class="record-sub"><i class="fas fa-cross" style="font-size:0.55rem;margin-right:3px;"></i><?= htmlspecialchars($rec['minister']) ?></div>
                        <?php endif; ?>
                    </td>
                    <td style="font-size:0.8rem;color:#374151;white-space:nowrap;">
                        <?= $rec['date_of_death'] ? date('M j, Y', strtotime($rec['date_of_death'])) : '<span style="color:#c4b89a;">—</span>' ?>
                    </td>
                    <td style="font-size:0.8rem;color:#374151;white-space:nowrap;">
                        <?= $rec['date_of_funeral'] ? date('M j, Y', strtotime($rec['date_of_funeral'])) : '<span style="color:#c4b89a;">—</span>' ?>
                    </td>
                    <td>
                        <?php $kin = $rec['next_of_kin_name'] ?: $rec['next_of_kin']; ?>
                        <?php if ($kin): ?>
                        <div style="font-size:0.8rem;color:#374151;"><?= htmlspecialchars($kin) ?></div>
                        <?php if ($rec['next_of_kin_contact']): ?>
                        <div class="record-sub"><i class="fas fa-phone" style="font-size:0.55rem;margin-right:3px;"></i><?= htmlspecialchars($rec['next_of_kin_contact']) ?></div>
                        <?php endif; ?>
                        <?php else: ?>
                    ...
                    <td style="font-size:0.78rem;color:#6b7280;">
                        <?= $rec['place_of_burial'] ? htmlspecialchars($rec['place_of_burial']) : '<span style="color:#c4b89a;">—</span>' ?>
                    </td>
                    <td>
                    ...
                    <td style="text-align:center;white-space:nowrap;">
                        <div style="display:flex;align-items:center;justify-content:center;gap:5px;">
                            <a href="/church/modules/church_records/funeral/view_record.php?id=<?= $rec['id'] ?>" class="action-btn action-view"><i class="fas fa-eye"></i> View</a>
                            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                            <a href="/church/modules/church_records/funeral/edit_record.php?id=<?= $rec['id'] ?>" class="action-btn action-edit"><i class="fas fa-pen"></i></a>
                            <a href="/church/modules/church_records/funeral/print_certificate.php?id=<?= $rec['id'] ?>" class="action-btn action-print" target="_blank"><i class="fas fa-print"></i></a>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
        ...
        <div class="pagination">
            ...
            <div class="pagination-links">
                <a href="<?= page_url($page_num - 1, $year, $search, $filter_doc, $sort) ?>" class="page-btn <?= $page_num <= 1 ? 'disabled' : '' ?>">
                    <i class="fas fa-chevron-left" style="font-size:0.65rem;"></i>
                </a>
                <?php
                $ws = max(1, $page_num - 2);
                $we = min($total_pages, $page_num + 2);
                if ($ws > 1): ?><a href="<?= page_url(1, $year, $search, $filter_doc, $sort) ?>" class="page-btn">1</a><?php if ($ws > 2): ?><span class="page-btn" style="pointer-events:none;border:none;color:#c4b89a;">…</span><?php endif; endif; ?>
                <?php for ($i = $ws; $i <= $we; $i++): ?>
                <a href="<?= page_url($i, $year, $search, $filter_doc, $sort) ?>" class="page-btn <?= $i === $page_num ? 'active' : '' ?>"><?= $i ?></a>
                <?php endfor; ?>
                <?php if ($we < $total_pages): ?><?php if ($we < $total_pages - 1): ?><span class="page-btn" style="pointer-events:none;border:none;color:#c4b89a;">…</span><?php endif; ?><a href="<?= page_url($total_pages, $year, $search, $filter_doc, $sort) ?>" class="page-btn"><?= $total_pages ?></a><?php endif; ?>
                <a href="<?= page_url($page_num + 1, $year, $search, $filter_doc, $sort) ?>" class="page-btn <?= $page_num >= $total_pages ? 'disabled' : '' ?>">
