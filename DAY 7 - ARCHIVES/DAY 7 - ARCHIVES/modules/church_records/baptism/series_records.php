<?php
// church/modules/church_records/baptism/series_records.php
// Phase 4 — Step 4.4: Records list inside a baptism year series
...
$s = $conn->prepare("SELECT id, notes FROM baptism_series WHERE series_year = ?");
$s->bind_param("i", $year);
$s->execute();
...
if (!$series) {
    $_SESSION['error'] = "No baptism series found for {$year}.";
    header('Location: /church/modules/church_records/baptism/series_list.php');
    exit;
}

$page_title = "Baptism Records — {$year}";
...
// ── Build WHERE clause ───────────────────────────────────────
$where_parts = ["br.is_archived = 0", "br.series_year = ?"];
$params      = [$year];
$types       = "i";

if ($search !== '') {
    $where_parts[] = "(br.child_name LIKE ? OR br.record_no LIKE ? OR br.father_name LIKE ? OR br.mother_name LIKE ?)";
    $like = "%{$search}%";
...
if ($filter_doc === 'yes') {
    $where_parts[] = "(br.document_path IS NOT NULL AND br.document_path != '')";
} elseif ($filter_doc === 'no') {
    $where_parts[] = "(br.document_path IS NULL OR br.document_path = '')";
}
...
$order_sql = match($sort) {
    'name_asc'   => 'br.child_name ASC',
    'name_desc'  => 'br.child_name DESC',
    'date_asc'   => 'br.date_of_baptism ASC',
    default      => 'br.date_of_baptism DESC',
};

// ── Total count for pagination ───────────────────────────────
$count_stmt = $conn->prepare("SELECT COUNT(*) FROM baptism_records br WHERE {$where_sql}");
...
$stats_r = $conn->prepare("
    SELECT
        ...
        MIN(date_of_baptism)                                            AS first_date,
        MAX(date_of_baptism)                                            AS last_date
...
<style>
    ...
    .shc-stat-value {
        font-family: 'Playfair Display', serif;
        font-size: 1.55rem; color: #93c5fd; font-weight: 600; line-height: 1;
    }
    .shc-stat-label {
    ...
    .scan-strip-pct {
        font-size: 0.8rem; font-weight: 600; color: #3b82f6; white-space: nowrap;
    }
    ...
    .records-table-title i { color: #3b82f6; font-size: 0.8rem; }
    .records-count-badge {
    ...
    .record-name {
        font-weight: 600; color: #0f2044; font-size: 0.855rem;
    }
    .record-sub {
    ...
    .empty-icon {
        ...
        background: #eff6ff;
        display: flex; align-items: center; justify-content: center;
        margin: 0 auto 20px; color: #3b82f6; font-size: 2rem;
    }
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    ...
    <a href="/church/modules/church_records/baptism/add_record.php?year=<?= $year ?>"
...
<div style="padding:24px 28px 60px; width:100%; box-sizing:border-box;">
    ...
    <div class="series-header-card">
        <div style="display:flex;align-items:center;gap:18px;">
            <div class="shc-icon"><i class="fas fa-droplet"></i></div>
            <div>
                <div class="shc-title"><?= $year ?> Baptism Series</div>
                <div class="shc-sub">
        ...
        <div class="toolbar">
            <div class="search-wrap">
                ...
                <input type="text"
                       name="q"
                       class="search-input"
                       placeholder="Search by name, record no., parent…"
            ...
            <button type="submit" class="toolbar-btn btn-blue">
    ...
    <div class="records-table-wrap">
        ...
        <div class="empty-state">
            ...
            <?php if ($search !== '' || $filter_doc !== ''): ?>
            <h3 style="font-family:'Playfair Display',serif;font-size:1rem;color:#0f2044;margin-bottom:8px;">
                No matching records
            </h3>
            <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;">
                No baptism records match your search. Try adjusting your filters.
            </p>
            ...
            <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;max-width:340px;margin-left:auto;margin-right:auto;line-height:1.6;">
                This series is empty. Add the first baptism record for <?= $year ?>.
            </p>
            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
            <a href="/church/modules/church_records/baptism/add_record.php?year=<?= $year ?>"
               class="toolbar-btn btn-blue" style="display:inline-flex;">
        ...
        <div style="width:100%;overflow-x:auto;">
            <table class="table">
                ...
                <tr>
                    ...
                    <!-- Child name -->
                    <td>
                        <div class="record-name"><?= htmlspecialchars($rec['child_name']) ?></div>
                    ...
                    <td style="font-size:0.8rem;color:#374151;white-space:nowrap;">
                        <?= $rec['date_of_baptism']
                            ? date('M j, Y', strtotime($rec['date_of_baptism']))
                            : '<span style="color:#c4b89a;">—</span>' ?>
                    </td>

                    <!-- Parents -->
                    <td>
                        <?php if ($rec['father_name'] || $rec['mother_name']): ?>
                        <div style="font-size:0.8rem;color:#374151;">
                            <?php if ($rec['father_name']): ?>
                            <div><?= htmlspecialchars($rec['father_name']) ?></div>
                            <?php endif; ?>
                            <?php if ($rec['mother_name']): ?>
                            <div style="color:#9ca3af;font-size:0.75rem;"><?= htmlspecialchars($rec['mother_name']) ?></div>
                            <?php endif; ?>
                        </div>
                        <?php else: ?>
                    ...
                    <td>
                        <?php
                            $gf = $rec['godfather'] ?? null;
                            $gm = $rec['godmother'] ?? null;
                        ?>
                        <?php if ($gf || $gm): ?>
                        <div style="font-size:0.78rem;color:#6b7280;">
                            <?php if ($gf): ?><div><?= htmlspecialchars($gf) ?></div><?php endif; ?>
                            <?php if ($gm): ?><div><?= htmlspecialchars($gm) ?></div><?php endif; ?>
                        </div>
                        <?php else: ?>
                        <span style="color:#c4b89a;font-size:0.78rem;">—</span>
                        <?php endif; ?>
                    ...
                    <td style="text-align:center;white-space:nowrap;">
                        <div style="display:flex;align-items:center;justify-content:center;gap:5px;">
                            <a href="/church/modules/church_records/baptism/view_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-view" title="View">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                            <a href="/church/modules/church_records/baptism/edit_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-edit" title="Edit">
                                <i class="fas fa-pen"></i>
                            </a>
                            <a href="/church/modules/church_records/baptism/print_certificate.php?id=<?= $rec['id'] ?>"
