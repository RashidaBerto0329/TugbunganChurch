<?php
// church/modules/church_records/funeral/series_list.php
...
$series_list = $conn->query("
    SELECT
        ...
        COUNT(CASE WHEN fr.document_path IS NOT NULL
                    AND fr.document_path != '' THEN 1 END)                     AS scanned_count
    FROM funeral_series fs
    LEFT JOIN funeral_records fr
        ON fr.series_year = fs.series_year AND fr.is_archived = 0
    GROUP BY fs.id, fs.series_year, fs.notes, fs.created_at
    ORDER BY fs.series_year DESC
...
$page_title = "Funeral Records — Year Series";
...
<style>
    ...
    .series-card-body {
        padding: 16px 20px 14px; flex: 1;
        display: flex; flex-direction: column; gap: 12px;
    }
    .series-stats-row { display: flex; gap: 0; }
    .series-stat {
        flex: 1; display: flex; flex-direction: column; gap: 2px;
        padding-right: 14px; border-right: 1px solid #f3ede3;
    }
    .series-stat:last-child { border-right: none; padding-right: 0; padding-left: 14px; }
    .series-stat-value {
        font-family: 'Playfair Display', serif;
        font-size: 1.35rem; font-weight: 600; color: #0f2044; line-height: 1;
    }
    .series-stat-label {
        font-size: 0.68rem; color: #c4b89a;
    ...
    .series-card-footer {
        padding: 12px 20px; border-top: 1px solid #f3ede3;
        display: flex; align-items: center;
        justify-content: space-between; gap: 10px;
    }
    .series-footer-date {
        font-size: 0.72rem; color: #c4b89a;
    ...
    .series-card:hover .series-open-btn { background: #4b5563; color: #fff; }
    ...
    .series-card-actions button:hover {
        text-decoration: underline;
    }

    /* ── New series CTA card ── */
    .series-card-new {
        background: #faf7f0; border: 2px dashed #d4c9b5;
    ...
    .series-card-new:hover { border-color: #6b7280; background: #f3f4f6; }
    .series-card-new-icon {
        width: 52px; height: 52px; border-radius: 14px;
        background: #ede8de; display: flex; align-items: center;
        justify-content: center; color: #9ca3af; font-size: 1.3rem;
        transition: background 0.2s, color 0.2s;
    }
    .series-card-new:hover .series-card-new-icon { background: #e5e7eb; color: #4b5563; }
    .series-card-new-title {
        font-family: 'Playfair Display', serif; font-size: 0.95rem;
        font-weight: 600; color: #9ca3af; transition: color 0.2s;
    }
    .series-card-new:hover .series-card-new-title { color: #4b5563; }
    .series-card-new p {
        font-size: 0.75rem; color: #c4b89a;
        text-align: center; line-height: 1.45; transition: color 0.2s;
    }
    ...
    .form-input {
        width: 100%; padding: 9px 12px; border: 1px solid #ede8de;
        border-radius: 8px; font-size: 0.875rem; color: #1a1a2e;
        outline: none; transition: border-color 0.18s, box-shadow 0.18s; background: #fff;
        box-sizing: border-box;
    }
    .form-input:focus { border-color: #6b7280; box-shadow: 0 0 0 3px rgba(107,114,128,0.1); }
    .form-hint { font-size: 0.72rem; color: #9ca3af; margin-top: 5px; }
    .modal-footer {
        padding: 16px 24px; border-top: 1px solid #f3ede3;
        display: flex; justify-content: flex-end; gap: 10px;
    }
    .btn-cancel {
        padding: 8px 18px; border-radius: 8px; font-size: 0.83rem;
        font-weight: 500; border: 1px solid #ede8de; background: #fff;
        color: #6b7280; cursor: pointer; transition: all 0.15s;
    }
    .btn-cancel:hover { background: #f9fafb; border-color: #c4b89a; }
    .btn-submit {
        padding: 8px 20px; border-radius: 8px; font-size: 0.83rem;
        font-weight: 600; background: #4b5563; color: #fff; border: none;
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    <div>
        <h1><i class="fas fa-cross" style="color:#6b7280;margin-right:8px;font-size:1rem;"></i>Funeral Records</h1>
        <p class="breadcrumb">
            ...
            <span style="margin:0 6px;color:#d1c9b8;">›</span>
            <span class="current">Funeral</span>
        </p>
    </div>
    ...
    <button onclick="openModal()"
            style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;
                   ...
                   background:#4b5563;color:#fff;cursor:pointer;transition:background 0.15s;">
...
<div style="padding:24px 24px 60px;">
    ...
    <!-- Summary banner -->
    <div class="funeral-banner">
        <div style="display:flex;align-items:center;gap:18px;">
            <div style="width:52px;height:52px;border-radius:14px;background:rgba(255,255,255,0.1);
                        display:flex;align-items:center;justify-content:center;
            ...
            <div>
                ...
                <p style="font-size:0.78rem;color:rgba(255,255,255,0.4);">
    ...
    <p style="font-family:'Playfair Display',serif;font-size:0.88rem;font-weight:600;
        ...
        <i class="fas fa-layer-group" style="color:#6b7280;font-size:0.8rem;"></i>
        Year Series
    </p>
    <p style="font-size:0.78rem;color:#9ca3af;margin-bottom:18px;">
        Select a year to view, add, or manage funeral records.
    </p>

    <?php if (empty($series_list)): ?>
    <div style="background:#fff;border:1px solid #ede8de;border-radius:14px;
                text-align:center;padding:60px 24px;">
        <div style="width:72px;height:72px;border-radius:20px;background:#f3f4f6;
                    display:flex;align-items:center;justify-content:center;
                    margin:0 auto 20px;color:#9ca3af;font-size:2rem;">
            <i class="fas fa-cross"></i>
        </div>
        <h3 style="font-family:'Playfair Display',serif;font-size:1.1rem;color:#0f2044;margin-bottom:8px;">
            No Funeral Records Yet
        </h3>
        <p style="font-size:0.85rem;color:#9ca3af;margin-bottom:24px;
                  max-width:380px;margin-left:auto;margin-right:auto;line-height:1.6;">
            Create your first year series to start recording funeral sacraments.
        </p>
        <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
        <button onclick="openModal()"
    ...
    <div class="series-grid">
        <?php foreach ($series_list as $s):
            $pct = $s['total_records'] > 0
                ? round(($s['scanned_count'] / $s['total_records']) * 100) : 0;
            $last_upd = $s['created_at']
                ? date('M j, Y', strtotime($s['created_at'])) : '—';
        ?>
        <div class="series-grid-item">
            <a href="/church/modules/church_records/funeral/series_records.php?year=<?= $s['series_year'] ?>"
               class="series-card">
                <div class="series-card-head" data-year="<?= $s['series_year'] ?>">
                    <div class="series-year-label"><?= $s['series_year'] ?></div>
                    <div class="series-sub-label">Funeral Series</div>
                ...
                <div class="series-card-footer">
                    <div class="series-footer-date">
                        <i class="fas fa-clock"></i> Created <?= $last_upd ?>
                    </div>
            ...
            <div class="series-card-actions">
                <button type="button"
                        onclick="openEditSeriesModal(<?= (int)$s['series_year'] ?>, '<?= htmlspecialchars($s['notes'] ?? '', ENT_QUOTES) ?>')">
...
<div class="modal-overlay" id="addSeriesModal">
    <div class="modal-box">
        ...
        <form method="POST" action="/church/modules/church_records/funeral/add_series.php">
            <div class="modal-body">
                ...
                <div style="background:#fef3c7;border:1px solid #fde68a;border-radius:8px;
                            padding:10px 14px;margin-top:14px;display:flex;align-items:center;gap:9px;">
...
<div class="modal-overlay" id="editSeriesModal">
    <div class="modal-box">
        <div class="modal-header">
            ...
            <button class="modal-close" onclick="closeEditSeriesModal()">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <form method="POST" action="/church/modules/church_records/funeral/edit_series.php">
            <div class="modal-body">
                ...
                <div class="form-group" style="margin-bottom:0%;">
                    <label class="form-label">Notes <span style="color:#9ca3af;font-weight:400;">(optional)</span></label>
                    <textarea name="notes" id="edit_series_notes" class="form-input" rows="3"
                              style="resize:vertical;"></textarea>
                </div>
            </div>
            <div class="modal-footer" style="justify-content:space-between;align-items:center;">
                <button type="submit" name="action" value="delete"
                        style="padding:7px 14px;border-radius:8px;border:1px solid #fecaca;
                               background:#fef2f2;color:#b91c1c;font-size:0.78rem;font-weight:600;cursor:pointer;"
                        onclick="return confirm('Are you sure you want to remove this series? It must have no funeral records.');">
...
<script>
    function openModal() {
        document.getElementById('addSeriesModal').classList.add('show');
        document.body.style.overflow = 'hidden';
    }
    function closeModal() {
        document.getElementById('addSeriesModal').classList.remove('show');
        document.body.style.overflow = '';
    }
    function openEditSeriesModal(year, notes) {
        const overlay = document.getElementById('editSeriesModal');
    ...
    document.getElementById('editSeriesModal')?.addEventListener('click', function(e) {
        if (e.target === this) closeEditSeriesModal();
    });
