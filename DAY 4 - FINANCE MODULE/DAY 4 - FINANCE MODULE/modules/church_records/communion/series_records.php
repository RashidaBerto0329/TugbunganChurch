<?php
// church/modules/church_records/communion/series_records.php
// Communion Module: Records list inside a communion year series
...
$s = $conn->prepare("SELECT id, notes FROM communion_series WHERE series_year = ?");
$s->bind_param("i", $year);
$s->execute();
...
if (!$series) {
    $_SESSION['error'] = "No communion series found for {$year}.";
    header('Location: /church/modules/church_records/communion/series_list.php');
    exit;
}

$page_title = "Communion Records — {$year}";
...
$order_sql = match($sort) {
    ...
    'date_asc'   => 'cr.date_of_communion ASC',
    default      => 'cr.date_of_communion DESC',
};

$count_stmt = $conn->prepare("SELECT COUNT(*) FROM communion_records cr WHERE {$where_sql}");
...
$fetch_sql = "
    SELECT
        ...
        cr.date_of_communion, cr.sponsor, cr.minister,
...
$stats_r = $conn->prepare("
    SELECT
        ...
        MIN(date_of_communion) AS first_date,
        MAX(date_of_communion) AS last_date
    FROM communion_records
...
<style>
    .series-header-card {
        ...
        position: relative; overflow: hidden; box-shadow: 0 4px 20px rgba(4,120,87,0.25);
    }
    .series-header-card::after {
        content: '<?= $year ?>'; position: absolute; right: 24px; top: 50%;
        transform: translateY(-50%); font-family: 'Playfair Display', serif;
        font-size: 5.5rem; font-weight: 700; color: rgba(255,255,255,0.06);
    ...
    .shc-stat-value { font-family:'Playfair Display',serif;font-size:1.55rem;color:#d1fae5;font-weight:600;line-height:1; }
    ...
    .scan-strip-fill { height:100%;background:linear-gradient(to right,#059669,#34d399);border-radius:99px;transition:width 0.5s ease; }
    .scan-strip-pct { font-size:0.8rem;font-weight:600;color:#059669;white-space:nowrap; }
    ...
    .btn-pink { background:#059669;color:#fff; }
    .btn-pink:hover { background:#047857; }
    ...
    .records-count-badge { display:inline-flex;align-items:center;font-size:0.72rem;font-weight:600;background:#f0fdf4;color:#059669;padding:2px 9px;border-radius:99px; }
    ...
    .action-view  { color:#059669;background:#f0fdf4; }
    .action-view:hover { background:#059669;color:#fff; }
    ...
    .empty-icon { width:72px;height:72px;border-radius:20px;background:#f0fdf4;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;color:#059669;font-size:2rem; }
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    ...
    <a href="/church/modules/church_records/communion/add_record.php?year=<?= $year ?>"
       class="toolbar-btn btn-pink">
...
<div style="padding:24px 28px 60px;width:100%;box-sizing:border-box;">
    ...
    <div class="series-header-card">
        <div style="display:flex;align-items:center;gap:18px;">
            <div class="shc-icon"><i class="fas fa-bread-slice"></i></div>
            <div>
                <div class="shc-title"><?= $year ?> Communion Series</div>
                <div class="shc-sub">
    ...
    <div class="scan-strip">
        <div style="font-size:0.78rem;color:#6b7280;white-space:nowrap;display:flex;align-items:center;gap:6px;">
            <i class="fas fa-file-image" style="color:#059669;"></i> Document scan coverage
        </div>
    ...
    <form method="GET" action="" id="filterForm">
        ...
        <div class="toolbar">
            ...
            <button type="submit" class="toolbar-btn btn-pink">
    ...
    <div class="records-table-wrap">
        <div class="records-table-header">
            <div class="records-table-title">
                <i class="fas fa-list" style="color:#059669;font-size:0.8rem;"></i>
                Communion Records
        ...
        <div class="empty-state">
            <div class="empty-icon"><i class="fas fa-bread-slice"></i></div>
            <?php if ($search !== '' || $filter_doc !== ''): ?>
            <h3 style="font-family:'Playfair Display',serif;font-size:1rem;color:#0f2044;margin-bottom:8px;">No matching records</h3>
            <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;">No communion records match your search. Try adjusting your filters.</p>
            ...
            <p style="font-size:0.82rem;color:#9ca3af;margin-bottom:20px;max-width:340px;margin-left:auto;margin-right:auto;line-height:1.6;">This series is empty. Add the first communion record for <?= $year ?>.</p>
            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
            <a href="/church/modules/church_records/communion/add_record.php?year=<?= $year ?>"
               class="toolbar-btn btn-pink" style="display:inline-flex;">
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <td style="font-size:0.8rem;color:#374151;white-space:nowrap;">
                        <?= $rec['date_of_communion']
                            ? date('M j, Y', strtotime($rec['date_of_communion']))
                    ...
                    <td style="text-align:center;white-space:nowrap;">
                        <div style="display:flex;align-items:center;justify-content:center;gap:5px;">
                            <a href="/church/modules/church_records/communion/view_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-view" title="View">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <?php if (in_array($current_user_role, ['admin', 'clergy'])): ?>
                            <a href="/church/modules/church_records/communion/edit_record.php?id=<?= $rec['id'] ?>"
                               class="action-btn action-edit" title="Edit">
                                <i class="fas fa-pen"></i>
                            </a>
                            <a href="/church/modules/church_records/communion/print_certificate.php?id=<?= $rec['id'] ?>"
