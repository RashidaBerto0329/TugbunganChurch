<?php
// church/modules/church_records/baptism/add_series.php
// Phase 4 — Step 4.3: Handle new year series form submission
...
// We check the baptism_series table if it exists, or we just check
// if any records already exist for that year and redirect gracefully.
// Using a dedicated baptism_series table for proper series management:
$check = $conn->prepare("SELECT id FROM baptism_series WHERE series_year = ?");
...
// ── Insert new series ─────────────────────────────────────────
$stmt = $conn->prepare("
    INSERT INTO baptism_series (series_year, notes, created_by, created_at)
    VALUES (?, ?, ?, NOW())
");
$stmt->bind_param("isi", $series_year, $notes, $_SESSION['user_id']);

if ($stmt->execute()) {
    $_SESSION['success'] = "Baptism series for {$series_year} created successfully.";
    header("Location: /church/modules/church_records/baptism/series_records.php?year={$series_year}");
} else {
