<?php
// church/modules/church_records/confirmation/add_record.php
// Phase 4 — Step 4.9: Add new confirmation record
...
// Verify confirmation_series exists
$s = $conn->prepare("SELECT id FROM confirmation_series WHERE series_year = ?");
$s->bind_param("i", $year);
$s->execute();
...
if (!$series) {
    $_SESSION['error'] = "No confirmation series found for {$year}. Please create the series first.";
    header('Location: /church/modules/church_records/confirmation/series_list.php');
    exit;
}
...
$series_id = null;
$rs = $conn->prepare("SELECT id FROM record_series WHERE type = 'confirmation' AND year = ?");
...
if ($rs_row) {
    ...
    $ins_rs = $conn->prepare("INSERT INTO record_series (type, year, created_by) VALUES ('confirmation', ?, ?)");
...
$rn = $conn->prepare("SELECT COUNT(*) FROM confirmation_records WHERE series_year = ? AND is_archived = 0");
$rn->bind_param("i", $year);
$rn->execute();
$existing_count = (int)$rn->get_result()->fetch_row()[0];
$rn->close();
$next_record_no = $year . '-C-' . str_pad($existing_count + 1, 3, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($record_no !== '') {
        $dup = $conn->prepare("SELECT id FROM confirmation_records WHERE record_no = ? AND series_year = ? AND is_archived = 0");
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("
            INSERT INTO confirmation_records
                (series_year, series_id, record_no, name, date_of_confirmation, date_of_birth,
        ...
        $stmt->bind_param(
            "iisssssssssssssi",
            $year, $series_id, $record_no, $name, $date_of_confirmation, $date_of_birth,
            $father_name, $mother_name, $address, $sponsor, $minister,
            $baptism_date, $baptism_parish, $remarks, $document_path,
            $current_user_id
        );
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            $_SESSION['success'] = "Confirmation record for \"{$name}\" added successfully.";
            header("Location: /church/modules/church_records/confirmation/view_record.php?id={$new_id}");
            exit;
        } else {
...
$page_title = "Add Confirmation Record — {$year}";
...
<style>
    ...
    .upload-area:hover,.upload-area.dragover { border-color:#8b5cf6;background:#f5f3ff; }
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    <div class="page-header-left">
        <h1>
            <i class="fas fa-plus" style="color:#8b5cf6;margin-right:8px;font-size:1rem;"></i>
...
<div style="display:grid;grid-template-columns:1fr 300px;gap:20px;align-items:start;" id="formLayout">
    ...
    <form method="POST" enctype="multipart/form-data" id="addRecordForm">
        ...
        <!-- SECTION 2: Confirmand Information -->
        <div class="form-card">
            <div class="form-card-header">
                <div class="form-card-header-icon" style="background:#f5f3ff;color:#8b5cf6;"><i class="fas fa-dove"></i></div>
                <div><div class="form-card-title">Confirmand Information</div><div style="font-size:0.72rem;color:#9ca3af;">Details of the person being confirmed</div></div>
            </div>
            <div class="form-card-body">
            ...
            <div class="form-card-body">
                <div class="form-grid-2">
                    <div class="form-group">
                        ...
                        <span class="form-hint">The sponsor (ninong/ninang) who accompanied the confirmand.</span>
                    </div>
                    <div class="form-group">
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div><div class="form-card-title">Baptism Reference</div><div style="font-size:0.72rem;color:#9ca3af;">Baptismal details of the confirmand (optional)</div></div>
            </div>
            <div class="form-card-body">
                <div class="form-grid-2">
                    <div class="form-group">
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <div><div class="form-card-title">Scanned Document</div><div style="font-size:0.72rem;color:#9ca3af;">Upload a photo or scan of the confirmation record</div></div>
            </div>
            <div class="form-card-body">
            ...
            <div class="form-card-body">
                <div class="form-group">
                    <textarea name="remarks" class="form-textarea" rows="3"
                              placeholder="Any additional notes or remarks about this confirmation record…"><?= htmlspecialchars($old['remarks'] ?? '') ?></textarea>
                </div>
            </div>
        </div>
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        <div style="background:#fff;border:1px solid #ede8de;border-radius:14px;overflow:hidden;">
            <div style="background:linear-gradient(135deg,#4c1d95,#7c3aed);padding:16px 18px;">
                ...
                <p style="font-size:0.72rem;color:rgba(255,255,255,0.45);">Confirmation Records</p>
            </div>
            <div style="padding:16px 18px;display:flex;flex-direction:column;gap:12px;">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <span style="font-size:0.78rem;font-weight:600;color:#8b5cf6;font-family:monospace;"><?= $next_record_no ?></span>
                </div>
                <hr style="border:none;border-top:1px solid #f3ede3;margin:0;">
                <a href="/church/modules/church_records/confirmation/series_records.php?year=<?= $year ?>"
                   style="display:flex;align-items:center;gap:7px;font-size:0.78rem;color:#8b5cf6;text-decoration:none;">
        ...
        <div style="background:#fff;border:1px solid #ede8de;border-radius:14px;padding:16px 18px;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
                <div style="display:flex;align-items:center;gap:8px;font-size:0.78rem;color:#374151;"><i class="fas fa-check-circle" style="color:#8b5cf6;font-size:0.75rem;"></i> Record Number</div>
                <div style="display:flex;align-items:center;gap:8px;font-size:0.78rem;color:#374151;"><i class="fas fa-check-circle" style="color:#8b5cf6;font-size:0.75rem;"></i> Date of Confirmation</div>
                <div style="display:flex;align-items:center;gap:8px;font-size:0.78rem;color:#374151;"><i class="fas fa-check-circle" style="color:#8b5cf6;font-size:0.75rem;"></i> Confirmand's Full Name</div>
            </div>
        ...
        <div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:14px;padding:16px 18px;">
            <p style="font-size:0.8rem;font-weight:600;color:#6d28d9;margin-bottom:8px;display:flex;align-items:center;gap:7px;">
                <i class="fas fa-dove" style="font-size:0.8rem;"></i> Confirmation Note
            </p>
            <p style="font-size:0.75rem;color:#5b21b6;line-height:1.55;">
                Include the sponsor's name and the officiating bishop or minister for a complete canonical record.
                The baptism reference section links this sacrament to the confirmand's earlier record.
            </p>
        </div>
    </div>
