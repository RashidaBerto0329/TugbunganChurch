<?php
// church/modules/church_records/communion/edit_record.php
// Communion Module: Edit an existing communion record
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($record_no !== '') {
        $dup = $conn->prepare("SELECT id FROM communion_records WHERE record_no = ? AND series_year = ? AND is_archived = 0 AND id != ?");
    ...
    if (empty($errors)) {
        ...
        $upd->bind_param(
            "sssssssssssssii",
            $record_no, $name, $date_of_communion, $date_of_birth,
        ...
        if ($upd->execute()) {
            ...
            header("Location: /church/modules/church_records/communion/view_record.php?id={$id}");
            exit;
        } else {
...
<style>
    ...
    .upload-area:hover { border-color:#059669;background:#f0fdf4; }
...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;">
    <div class="page-header-left">
        <h1><i class="fas fa-pen" style="color:#b8933a;margin-right:8px;font-size:1rem;"></i>Edit Communion Record</h1>
        <p class="breadcrumb">
            ...
            <span style="margin:0 6px;color:#d1c9b8;">›</span>
            <a href="/church/modules/church_records/communion/view_record.php?id=<?= $id ?>" style="color:#9ca3af;text-decoration:none;"><?= htmlspecialchars($rec['name']) ?></a>
            <span style="margin:0 6px;color:#d1c9b8;">›</span>
            <span class="current">Edit</span>
        </p>
    </div>
    <a href="/church/modules/church_records/communion/view_record.php?id=<?= $id ?>" class="btn-cancel-link">
        ...
        <form method="POST" enctype="multipart/form-data" id="editForm">
                    ...
                    <div class="form-grid-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="date_of_communion" class="form-input"
                                   ...
                                   data-original="<?= htmlspecialchars($rec['date_of_communion'] ?? '') ?>">
                        </div>
                    </div>
                </div>
            </div>

            <!-- SECTION 2: Communicant Info -->
            <div class="form-card">
                <div class="form-card-header">
                    <div class="form-card-header-icon" style="background:#f0fdf4;color:#059669;"><i class="fas fa-bread-slice"></i></div>
                    <div><div class="form-card-title">Communicant Information</div><div style="font-size:0.72rem;color:#9ca3af;">Details of the person who received First Communion</div></div>
                </div>
                <div class="form-card-body">
            ...
            <div class="form-card">
                <div class="form-card-header">
                    ...
                    <div><div class="form-card-title">Baptism Reference</div><div style="font-size:0.72rem;color:#9ca3af;">Baptismal details of the communicant</div></div>
                </div>
                <div class="form-card-body">
                    <div class="form-grid-2">
                        <div class="form-group">
            ...
            <div class="form-card">
                ...
                <div class="form-card-body">
                    ...
                    <div class="existing-doc" id="existingDoc">
                        ...
                        <span style="font-size:0.82rem;font-weight:500;color:#15803d;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= basename($rec['document_path']) ?></span>
                        <a href="<?= htmlspecialchars($rec['document_path']) ?>" target="_blank" style="font-size:0.75rem;color:#16a34a;text-decoration:none;white-space:nowrap;">
                    ...
                    <p style="font-size:0.75rem;color:#9ca3af;margin-bottom:10px;">Upload a new file below to <strong>replace</strong> the existing document, or check "Remove" to delete it.</p>
                    <?php endif; ?>
                    ...
                    <div class="upload-preview" id="uploadPreview">
                        ...
                        <button type="button" style="background:none;border:none;color:#dc2626;cursor:pointer;font-size:0.8rem;" onclick="clearFile()"><i class="fas fa-times"></i></button>
                    </div>
                </div>
            </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;position:sticky;top:20px;">
            <div style="background:#fff;border:1px solid #ede8de;border-radius:14px;overflow:hidden;">
                <div style="background:linear-gradient(135deg,#065f46,#059669);padding:16px 18px;">
                    ...
                    <p style="font-size:0.72rem;color:rgba(255,255,255,0.45);"><?= $year ?> Communion Series</p>
                </div>
                <div style="padding:14px 18px;">
                    <div class="info-row"><span class="info-row-label">Record No.</span><span class="info-row-value" style="font-family:monospace;color:#059669;"><?= htmlspecialchars($rec['record_no']) ?></span></div>
                    <div class="info-row"><span class="info-row-label">Name</span><span class="info-row-value" style="max-width:140px;text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($rec['name']) ?></span></div>
                    <div class="info-row"><span class="info-row-label">Series Year</span><span class="info-row-value"><?= $year ?></span></div>
                    <div class="info-row"><span class="info-row-label">Document</span><span class="info-row-value">
...
const labels = {
    ...
    date_of_communion: "Communion Date", date_of_birth: "Date of Birth",
...
uploadArea.addEventListener('dragover',  e => { e.preventDefault(); uploadArea.style.borderColor = '#059669'; });
