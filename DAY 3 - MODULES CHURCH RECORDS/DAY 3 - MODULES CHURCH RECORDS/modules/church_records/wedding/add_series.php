<?php
// church/modules/church_records/wedding/add_series.php
...
$dup = $conn->prepare("SELECT id FROM wedding_series WHERE series_year = ?");
...
// Insert into wedding_series
$ins = $conn->prepare("INSERT INTO wedding_series (series_year, notes, created_by) VALUES (?, ?, ?)");
$ins->bind_param("isi", $year, $notes, $current_user_id);
$ins->execute();
$ins->close();

// Also insert into the central record_series registry (if not already there)
$rs_check = $conn->prepare("SELECT id FROM record_series WHERE type = 'wedding' AND year = ?");
...
$_SESSION['success'] = "Wedding series for {$year} created successfully.";
header("Location: /church/modules/church_records/wedding/series_records.php?year={$year}");
exit;